...
<div class="form-control" style=" border:none; padding-left: 0;">
    ...
    <script type="text/javascript">
        $(document).on("click",".btn-clone-item",function(){
        ...
        $(document).on("click",".btn-clone-item-delete",function(e){
            e.preventDefault();
    ...
    <div class="list_multifields">
        ...
        <div class="list-cloned-items" id="listAddons">
            @foreach($options['values'] as $idx => $val)
            <div class="item row">
                <div class="col-lg-5">  
                    <input type="text" class="field" name="addons[{{$idx}}][name]" value="{{$val['name']}}" />
                </div>
                <div class="col-lg-2">
                    <input type="text" class="field" name="addons[{{$idx}}][price]" value="{{$val['price']}}" />
                </div>
                <div class="col-lg-2">
                    <a class="btn-clone-item-delete"><i class="fa fa-trash btn-remove"></i></a>
                </div>
            </div>
            @endforeach
            <div class="clone row d-none">
                <div class="col-lg-5">  
                    <input type="text" class="field" disabled="disabled" name="addons[X][name]" />
                </div>
                <div class="col-lg-2">
                    <input type="text" class="field" disabled="disabled" name="addons[X][price]" value="0.00" />
                </div>
                <div class="col-lg-2">
                    <a class="btn-clone-item-delete"><i class="fa fa-trash btn-remove"></i></a>
                </div>
        ...
        <button type="button" style="margin-left: 10px;" class="btn btn-success btn-clone-item" data-target="listAddons" data-index="[X]">Add New</button>
    </div>  

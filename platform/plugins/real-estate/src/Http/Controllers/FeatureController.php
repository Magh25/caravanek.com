...
class FeatureController extends BaseController
{
    ...
    **/
    ...
    {   
        ...
        if( !empty($feature->id) && !empty($request->get('name')) && empty($feature->name) ){
            $rdata = []; 
            foreach($request->all() as $f => $v){
                if( is_string($v) && in_array($f,['name','icon','type','group','select_options','show_on_listing','order']) ){
                    $feature->$f = $v;
                    $rdata[$f] = $v;
                }
            }
            \DB::table('re_features')->where('id', $feature->id)->update($rdata);
        }
        
        event(new CreatedContentEvent(FEATURE_MODULE_SCREEN_NAME, $request, $feature));
        
        if ($feature) {
            $feature->types()->sync($request->input('types', []));
        }

        return $response->setPreviousUrl(route('property_feature.index'))->setNextUrl(route('property_feature.edit', $feature->id))->setMessage(trans('core/base::notices.create_success_message'));
    }
    ...
    **/
    ...
    public function update($id, FeatureRequest $request, BaseHttpResponse $response)
    {
        ...
        do_action(ACTION_BEFORE_UPDATE_PROPERTY, $request, $feature);
        ...
        return $response->setPreviousUrl(route('property_feature.index'))->setMessage(trans('core/base::notices.update_success_message'));
    }

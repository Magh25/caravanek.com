    ...
    public function checkCurrentLanguage($languages, $value)
    {
        ...
        foreach ($languages as $language) {
            if ($value) {
                ...
                } else if( $request->input('language') ){
                    if ($language->lang_code == $request->input('language') )
                        $currentLanguage = $language;

                } else if( !empty(session('editing_language')) ){
                    if ($language->lang_code == session('editing_language')) 
                        $currentLanguage = $language;
    ...
    protected function getDataByCurrentLanguageCode($data, $languageCode)
    {
        ...
        if (in_array(get_class($model), Language::supportedModels()) &&
            ...
            if (Language::getCurrentAdminLocaleCode() !== 'all') {
                ...
                $data = $data
                    ->select('GROUP_CONCAT(language_meta.field) as fields')
                    ->leftJoin('language_meta', 'language_meta.reference_id', $table . '.id')
                    ->where('language_meta.reference_type', get_class($model))
                    ->where('language_meta.lang_meta_code', $languageCode)->groupBy($table . '.id');
            }

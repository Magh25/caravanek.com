...
class LanguageManager
{
    ...
    public static function getLanguageColumns(){
        return ['title','name','description','content','nationality'];
    }
    ...
    public function getCurrentAdminLocaleCode()
    {
        ...
        if ($this->request->has('language')) {
            return $this->request->input('language');
        }

        if( !empty(session('editing_language')) ){
            return session('editing_language');
        }
    ...
    public function saveLanguageCustom($screen, $request, $data){
        $defaultLanguage = $this->getDefaultLanguage(['lang_id']);
        if (!empty($defaultLanguage)) {
            if ($data != false && in_array(get_class($data), $this->supportedModels()) && $request->input('language') && !empty($data->id) ){
                
                $language       = trim($request->input('language'));
                $reference_id   = (int)$data->id;
                $reference_type = get_class($data);
                
                session(['editing_language' => $language]);

                $columns = self::getLanguageColumns();
                if( method_exists($data,'getLanguageColumns') )
                    $columns = $data->getLanguageColumns();
                
                $fields = [];
                foreach($columns as $col){
                    if( isset($data->$col) )
                        $fields[$col] = is_object($data->$col) || is_array($data->$col) ? json_encode($data->$col) : trim($data->$col); 
                }

                $origin_id = md5($reference_type.$reference_id.$language);

                \DB::table('language_meta')->where('lang_meta_origin',$origin_id)->delete();

                foreach($fields as $col => $value){
                    $metaData = [
                        'lang_meta_origin' => $origin_id,
                        'lang_meta_code' => $language,
                    ...
                    \DB::table('language_meta')->insert($metaData);
                }
    ...
    {
        return $this->saveLanguageCustom($screen, $request, $data);
